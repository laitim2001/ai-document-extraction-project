...
> **日期**: 2026-06-10
> **狀態**: ✅ 已完成（程式碼 + 本地 enforce 執行期驗收 2026-06-12 場景 1~5 全通過；production 生效僅需於 Azure 部署環境設 `API_AUTH_GATE_MODE=enforce`，純環境變數切換、無 code change，見「執行期驗收結果」）
...
### 執行期驗收結果（2026-06-12，本地 dev port 3200）

於本地 dev server 完成 `monitor` 與 `enforce` 兩種模式的對照驗收，5 項驗收標準全部通過。

#### monitor 模式（`API_AUTH_GATE_MODE` 未設）

對 7 個代表性端點發未登入請求，server log 出現 `[API-AUTH-GATE][monitor]` 警告：

| 端點 | gate 是否記錄 | 結論 |
|------|--------------|------|
| `/api/health`、`/api/openapi` | 否 | 公開白名單放行 |
| `/api/v1/invoices` | 否 | 對外 ApiKey 白名單放行（handler 回 `MISSING_API_KEY`）|
| `/api/documents`、`/api/companies`、`/api/cost/pricing`、`/api/v1/users/me` | 是（「enforce 模式下將回 401」）| 受保護端點正確識別；**白名單無遺漏** |

> monitor 模式下受保護端點仍回 401，但 body 為各 handler 自身格式（FIX-063~067 縱深防禦），證明 middleware 放行、handler 自擋。

#### enforce 模式（`API_AUTH_GATE_MODE=enforce`）

| # | 驗收標準 | 結果 | 證據 |
|---|----------|------|------|
| 1 | 未登入擋下 | ✅ | 受保護 `/api/*` → 401，body 為 middleware RFC 7807（`detail:"需要登入才能存取此資源"` + `instance`），證明 middleware 搶先攔截、請求未到 handler |
| 2 | 白名單放行 | ✅ | `/api/health`、`/api/openapi` → 200 |
| 3 | ApiKey 雙軌 | ✅ | `/api/v1/invoices` 放行給 handler，回 `MISSING_API_KEY`（body 與 monitor 完全相同，不受 mode 影響）|
| 4 | 登入迴歸 | ✅ | Playwright 實際登入後，已登入 session fetch `/api/documents`、`/api/companies`、`/api/cost/pricing`、`/api/v1/users/me` 全部 200 + 真實資料 |
| 5 | JSDoc 一致 | ✅ | middleware 註解與行為相符 |

#### Production 部署交付項

- 本地驗收證明 enforce 模式安全可用。**production / staging 生效僅需在 Azure 部署環境變數設 `API_AUTH_GATE_MODE=enforce`**（本變更不含 code change，純環境變數切換；`.env.example` 已含說明）。
- 上線前置（沿用既有）：確認各環境本地管理員密碼已從預設 `ChangeMe@2026!` 改掉（WP-7／INFRA-01）。

### 待後續

- ~~執行期驗收（場景 1~4）需起服務 / staging 驗證。~~ ✅ 已於 2026-06-12 本地完成（見上方「執行期驗收結果」）。
