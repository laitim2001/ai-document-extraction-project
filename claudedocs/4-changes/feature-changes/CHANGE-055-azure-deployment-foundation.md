...
1. DBA（Andy.kao）建立 `ai_document_extraction` DB + 給 `raposcmaidocprocessingdev` 完整權限
2. 建 ACR（Basic，admin-enabled）→ `az acr build` 雲端建置推送映像
3. 設定 WebApp 容器映像 + 所有 App Service 應用程式設定（DATABASE_URL / AI keys / storage / secrets / 模型名 / 管理員帳密 / NODE_ENV…）
4. 重啟 WebApp → 容器自動 bootstrap schema + seed
5. 冒煙測試（/api/health、admin 登入、上傳、提取）

### 用戶決策記錄（本 session）

| 決策 | 選擇 |
|------|------|
| 部署架構 | 沿用既有 App Service（非另建 Container Apps）|
