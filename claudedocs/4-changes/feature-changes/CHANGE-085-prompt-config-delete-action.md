...
| 檔案 | 修改內容 |
|------|----------|
| `src/components/features/prompt-config/PromptConfigList.tsx` | 函數簽名解構補 `onDelete`；渲染 `CollapsiblePromptGroup` 時往下傳 `onDelete` |
| `src/components/features/prompt-config/CollapsiblePromptGroup.tsx` | props 介面加 `onDelete?`；透傳給 `PromptConfigCard`；`DropdownMenuContent` 新增 destructive 刪除項；`config.isActive` 時 item `disabled` 並顯示 `list.deleteDisabledHint` 小字提示；import `Trash2` |
| `src/app/api/v1/prompt-configs/[id]/route.ts` | DELETE 在 `prisma.promptConfig.delete` 前加 `existingConfig.isActive` 檢查，啟用中回 409 + RFC 7807（top-level，與檔案既有錯誤格式一致） |
| `messages/{en,zh-TW,zh-CN}/promptConfig.json` | `list` 區塊新增 `delete` + `deleteDisabledHint`（三語言同步） |

### 設計符合度

- **回調鏈接通**：page.tsx（第 196 行 `onDelete={handleDeleteRequest}`、確認 `AlertDialog`、`useDeletePromptConfig`）原已完整，本次僅補回中間斷掉的 `PromptConfigList` → `CollapsiblePromptGroup` → 卡片透傳。
- **D7 雙防線**：後端 409 為真實防線（防直接打 API），前端 disabled + 提示為體驗防線，皆已實作。
- **onDelete optional**：未提供時不顯示刪除入口，向後相容。

### 驗證結果

- `npm run type-check`：通過（exit 0）
- `npm run lint`：通過（改動檔案無新 warning）
