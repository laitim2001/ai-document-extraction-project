...
1. **導出服務 (`template-export.service.ts`)**
   - Excel 導出 (exceljs)
   - CSV 導出 (UTF-8 BOM)
   - 行篩選 (all/valid/invalid)
   - 欄位選擇和排序
   - 日期/數字/金額格式化

2. **導出 API (`/api/v1/template-instances/[id]/export`)**
   - GET 方法，支援查詢參數
   - format: xlsx | csv
   - rowFilter: all | valid | invalid
   - fields: 欄位列表 (逗號分隔)
   - dateFormat: 日期格式
   - useThousandSeparator: 千分位
   - includeValidationErrors: 包含驗證錯誤

3. **導出對話框 (`ExportDialog.tsx`)**
   - 格式選擇 (Excel/CSV)
   - 行篩選選項
   - 進階格式化選項
   - 導出進度顯示

4. **欄位選擇器 (`ExportFieldSelector.tsx`)**
   - 全選/取消全選
   - 重置為預設
   - 欄位類型徽章
   - 必填欄位指示
...
#### 新增文件
- `src/services/template-export.service.ts`
- `src/app/api/v1/template-instances/[id]/export/route.ts`
- `src/components/features/template-instance/ExportDialog.tsx`
- `src/components/features/template-instance/ExportFieldSelector.tsx`

#### 修改文件
- `src/components/features/template-instance/TemplateInstanceDetail.tsx` (整合導出對話框)
- `messages/en/templateInstance.json` (新增 export 翻譯)
- `messages/zh-TW/templateInstance.json` (新增 export 翻譯)
- `messages/zh-CN/templateInstance.json` (新增 export 翻譯)
