...
- **Story 19-3**: TemplateMatchingEngineService
- **Story 19-2**: TemplateInstance
...
1. **Prisma Schema 更新**
   - Document 模型新增 templateInstanceId, templateMatchedAt 欄位
   - Company 模型新增 defaultTemplateId 欄位和 defaultTemplate 關聯
   - DocumentFormat 模型新增 defaultTemplateId 欄位和 defaultTemplate 關聯

2. **自動匹配服務 (`auto-template-matching.service.ts`)**
   - resolveDefaultTemplate：三層優先級解析 (FORMAT > COMPANY > GLOBAL)
   - autoMatch：處理完成後自動匹配
   - matchSingle：單一文件手動匹配
   - batchMatch：批量匹配（最多 500 個文件，進度回調）
   - unmatch/batchUnmatch：取消匹配
   - getMatchStatus：獲取匹配狀態
   - getUnmatchedCount：獲取未匹配數量

3. **API 端點**
   - POST `/api/v1/documents/match` - 批量匹配
   - POST `/api/v1/documents/[id]/match` - 單一匹配
   - POST `/api/v1/documents/[id]/unmatch` - 取消匹配

4. **UI 組件**
   - MatchToTemplateDialog - 手動匹配對話框
   - BulkMatchDialog - 批量匹配對話框（進度顯示）
   - DefaultTemplateSelector - 預設模版選擇器
   - MatchStatusBadge - 匹配狀態徽章

5. **i18n 翻譯更新**
   - templateInstance.json 新增 match/bulkMatch/unmatch/defaultTemplate sections
   - en/zh-TW/zh-CN 三語言支援

#### 新增文件
- `src/services/auto-template-matching.service.ts`
- `src/app/api/v1/documents/match/route.ts`
- `src/app/api/v1/documents/[id]/match/route.ts`
- `src/app/api/v1/documents/[id]/unmatch/route.ts`
- `src/components/features/template-match/MatchToTemplateDialog.tsx`
- `src/components/features/template-match/BulkMatchDialog.tsx`
- `src/components/features/template-match/DefaultTemplateSelector.tsx`
- `src/components/features/template-match/MatchStatusBadge.tsx`
- `src/components/features/template-match/index.ts`
- `src/validations/template-matching.ts`

#### 修改文件
- `prisma/schema.prisma` (Document, Company, DocumentFormat 新增欄位)
- `messages/en/templateInstance.json` (新增 match 翻譯)
- `messages/zh-TW/templateInstance.json` (新增 match 翻譯)
- `messages/zh-CN/templateInstance.json` (新增 match 翻譯)
...
- `src/app/api/v1/documents/match/route.ts` - 新增
- `src/app/api/v1/documents/[id]/match/route.ts` - 新增
- `src/app/api/v1/documents/[id]/unmatch/route.ts` - 新增
- `src/components/features/template-match/` - 新增目錄
