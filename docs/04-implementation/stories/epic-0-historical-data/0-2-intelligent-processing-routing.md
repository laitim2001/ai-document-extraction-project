...
**Then** 根據 `detectedType` 自動選擇：
  ...
  - `SCANNED_PDF` → GPT-5.2 Vision
  - `IMAGE` → GPT-5.2 Vision
...
- [ ] **Task 6: GPT-5.2 Vision 整合** (AC: #1, #3)
  - [ ] 6.1 創建 `src/services/gpt-vision.service.ts`
  - [ ] 6.2 圖片轉 base64
  - [ ] 6.3 GPT-5.2 Vision API 調用
  - [ ] 6.4 結果解析
...
- 需要 Azure OpenAI API Key（GPT-5.2 Vision）
