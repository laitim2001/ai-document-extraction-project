...
- [ ] **Task 1: 停用/啟用按鈕** (AC: #1, #2)
  - [ ] 1.1 在用戶列表加入操作按鈕
  - [ ] 1.2 根據用戶狀態顯示「停用」或「啟用」
  - [ ] 1.3 使用不同顏色區分操作
...
- [ ] **Task 3: 狀態變更 API** (AC: #1, #2)
  - [ ] 3.1 創建 PATCH `/api/admin/users/[id]/status` 端點
  - [ ] 3.2 更新用戶 status 欄位
  - [ ] 3.3 記錄審計日誌
  - [ ] 3.4 返回更新結果

- [ ] **Task 4: 登入阻擋邏輯** (AC: #3)
  - [ ] 4.1 更新 NextAuth signIn 回調
  - [ ] 4.2 檢查用戶 status 是否為 ACTIVE
  - [ ] 4.3 非 ACTIVE 狀態返回錯誤

- [ ] **Task 5: 停用用戶錯誤頁面** (AC: #3)
  - [ ] 5.1 更新 `/error` 頁面處理 AccountDisabled 錯誤
  - [ ] 5.2 顯示友善的停用訊息
  - [ ] 5.3 提供聯繫管理員的指引

- [ ] **Task 6: 用戶服務層擴展** (AC: #1, #2)
  - [ ] 6.1 實現 updateUserStatus 函數
  - [ ] 6.2 處理狀態變更邏輯
  - [ ] 6.3 觸發審計日誌記錄

- [ ] **Task 7: UI 狀態顯示** (AC: #1, #2)
  - [ ] 7.1 用戶列表顯示狀態 Badge
  - [ ] 7.2 Active 顯示綠色
  - [ ] 7.3 Inactive 顯示灰色
